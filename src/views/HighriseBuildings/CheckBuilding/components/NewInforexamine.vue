...
const rules = reactive({
  csqloginid: [{required: true, message: '请输入社区', trigger: 'blur'},],
  xqbh: [{required: true, message: '请输入小区（楼）名称', trigger: 'blur'},],
  lh: [{required: true, message: '请输入楼号', trigger: 'blur'},],
  szdz: [{required: true, message: '请输入所在地址', trigger: 'blur'},],
  jcr: [{required: true, message: '请输入检查人', trigger: 'blur'},],
  jcsj: [{required: true, message: '请输入检查时间', trigger: 'blur'},],
})
      ...
      BasicChecks.value.forEach((item) => {
        item.children.forEach((i) => {
          Information.jcxx.push({
            jcxm: i.dictName, jcjg: i.jcjg, fj: i.files && i.files.map((item) => {
  ...
  if (props.bh !== '') {
    ...
    const getinfos = async () => {
      ...
      data.checkItem.forEach((item) => {
        if (item.zdy) {
          custom.value.push({
            jcxm: item.jcxm, jcjg: item.jcjg, fj: item.zp.map((item) => {
              return {
                url: 'http://kfq.kejin.net.cn:8223' + item.path, response: {
                  data: item
                }
